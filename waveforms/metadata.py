...
def compute_offsets(cha, ref):
    (x, y) = util_geo_km(ref.longitude, ref.latitude, cha.longitude, cha.latitude)
    return (x, y)
...
def get_metadata(
    ...
    **kwargs,
    ...
    del invselect["local_storage"]
    del invselect["local_file"]
    del invselect["kwargs"]
    # print("get_metadata locals: \n", invselect)
    [(key, value)] = data_source.items()
    _invselect = {str(key): str(value) for key, value in invselect.items()}
    invselect = _invselect

    if key == "local":
        inv = read_inventory(value, "STATIONXML")
        del invselect["level"]  # <<--- TODO: check if this is allowed
        inv = inv.select(**invselect)

    elif key == "fdsn":
        cl = fdsn_client(value)
        inv = cl.get_stations(**invselect)
    ...
    if local_storage:
        print(f" -> Writing inventory to {local_file}...")
        inv.write(local_file, format="STATIONXML")
...
def inv_to_coords(inv):
    ...
    """
    n_channels = len(inv.get_contents()["channels"])
    coords = np.zeros((n_channels, 3))

    for i, seed_id in enumerate(inv.get_contents()["channels"]):
        sta_info = inv.get_coordinates(seed_id)
        coords[i, 0] = sta_info["latitude"]
        coords[i, 1] = sta_info["longitude"]
        coords[i, 2] = sta_info["local_depth"]
...
def inv_to_dtk_xml(inv, array_name):
    ...
    """
    ns = {
        None: "http://www.fdsn.org/xml/station/1",
        "dtk": "http://www.fdsn.org/xml/dtk/1",
    }

    date_fmt = "%Y-%m-%dT%H:%M:%S"
    root = ET.Element("FDSNStationXML", nsmap=ns, schemaVersion="1")
    ...
    for net in inv:
        ...
        station = ET.SubElement(
            network, "Station", code=f"{array_name}", startDate=f"{t0}", endDate=f"{t1}"
        )
        ...
        for item in inv.get_contents()["channels"]:
            ...
            invs = inv.select(network=net, station=sta, location=loc, channel=cha)
